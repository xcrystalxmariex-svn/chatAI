  ...
  const handleToolExecution = async (toolCalls: any[], conversationHistory: any[]) => {
    try {
      ...
      try {
        ...
        console.log('[ChatScreen] Database save failed:', dbError);
      }
      ...
      const toolResults = await Promise.all(
        toolCalls.map(async (toolCall) => {
          try {
            ...
            // Check if rate limited - don't loop
            const parsedResult = JSON.parse(result);
            if (parsedResult.rateLimited) {
              return {
                role: 'tool',
                content: JSON.stringify({ success: false, error: 'Tool rate limited' }),
            ...
            console.error(`[ChatScreen] Tool ${toolCall.name} execution failed:`, toolError);
            return {
              role: 'tool',
      ...
      // Send tool results back to AI - disable tools to prevent loops
      const finalResponse = await aiService.sendMessage(
        ...
        undefined,
        false // Disable tools for final response - CRITICAL to prevent loops
      );
      ...
      const finalMessage: Message = {
        ...
        role: 'assistant',
        content: finalResponse.content || 'Tool execution completed.',
        timestamp: Date.now(),
        conversationId: currentConversationId!,
      };
      ...
      console.error('[ChatScreen] Tool execution error:', error);
      
      // Show error message in chat instead of alert
      const errorMessage: Message = {
        id: (Date.now() + 3).toString(),
        role: 'assistant',
        content: `⚠️ Tool execution encountered an issue: ${error.message}. I'll try to help without using tools.`,
        timestamp: Date.now(),
        conversationId: currentConversationId!,
      };
